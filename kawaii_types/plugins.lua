...
---@class (exact) KwImportSpec: LazySpecImport
...
---     - If a function, then the second argument of the function are the list of key sequences that defined the loading of this plugin by Kawaii
...
--- If `opts` is a function, then the second argument `opts` is the default config defined by Kawaii
...
---     }
--- ```
---@field dependencies? (string[]|KwPluginSpec) 
...
--- - If a function, then the second argument of the function are the list of event(s) that defined the loading of this plugin by Kawaii (if any)
---     - The first argument is reserved for internal use, it should not be used under normal circumstances
---
...
--- By default, all Kawaii plugins have `lazy = true`, meaning it will not be loaded unless
...
--- By default, all Kawaii plugins has `lazy = true`, which means it will not be loaded automatically 
--- unless you specify an event for it to load, or to set `lazy = false` for it to load on startup
---@alias KwPluginSpec string|LazyPluginSpecExtra|KwImportSpec|KwPluginSpec[]
