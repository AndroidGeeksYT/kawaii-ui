...
local ch = require "kawaii.cheatsheet"
...
dofile(vim.g.base46_cache .. "kwcheatsheet")

api.nvim_create_autocmd("BufWinLeave", {
  callback = function()
    if vim.bo.ft == "kwcheatsheet" then
...
return function(buf, win, action)
  ...
  local ns = api.nvim_create_namespace "kwcheatsheet"
  ...
  local lines = {
    { genstr(" ", box_w), "KwChAsciiHeader" },
    { addpadding(heading[1]), "KwChAsciiHeader" },
    { addpadding(heading[2]), "KwChAsciiHeader" },
    { genstr(" ", box_w), "KwChAsciiHeader" },
    { "" },
  }
  ...
  for _, name in ipairs(sections) do
    table.insert(lines, { addpadding(name), "KwChheading" })
    table.insert(lines, { genstr(" ", box_w), "KwChSection" })
    ...
    for _, val in ipairs(state.mappings_tb[name]) do
      ...
      table.insert(lines, { str, "KwChSection" })
      table.insert(lines, { genstr(" ", #str), "KwChSection" })
    end
