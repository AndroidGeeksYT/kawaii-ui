...
return function(buf, win, action)
  ...
  for name, section in pairs(state.mappings_tb) do
    ...
    for _, mapping in ipairs(section) do
      ...
      table.insert(cards[name], { { emptyline, "kwchsection" } })
      ...
      table.insert(cards[name], { { "  " .. pretty_mapping .. "  ", "kwchsection" } })
    end
    table.insert(cards[name], { { emptyline, "kwchsection" } })
  ...
  for key, value in pairs(cards) do
    ...
    -- center the heading
    key = {
      { string.rep(" ", pad_l), "kwchsection" },
      { key, ch.rand_hlgroup() },
      { string.rep(" ", pad_r), "kwchsection" },
    }
