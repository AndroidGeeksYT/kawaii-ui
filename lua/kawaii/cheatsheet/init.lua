...
M.get_mappings = function(mappings, tb_to_add)
  local excluded_groups = require("kwconfig").cheatsheet.excluded_groups
...
M.organize_mappings = function()
  ...
  for _, mode in ipairs(modes) do
    ...
    require("kawaii.cheatsheet").get_mappings(keymaps, tb_to_add)

    local bufkeymaps = vim.api.nvim_buf_get_keymap(0, mode)
    require("kawaii.cheatsheet").get_mappings(bufkeymaps, tb_to_add)
  end
...
M.autocmds = function(buf)
  require("kawaii.utils").set_cleanbuf_opts("kwcheatsheet", buf)

  local group_id = api.nvim_create_augroup("KwCh", { clear = true })

  api.nvim_create_autocmd("BufWinLeave", {
    group = group_id,
    buffer = buf,
    callback = function()
      vim.g.kwcheatsheet_displayed = false
      api.nvim_del_augroup_by_name "KwCh"
    end,
  })
  ...
  api.nvim_create_autocmd({ "WinResized", "VimResized" }, {
    group = group_id,
    callback = function()
      require("kawaii.cheatsheet." .. config.cheatsheet.theme)(vim.g.kwch_buf, vim.g.kwch_win, "redraw")
    end,
  })
  ...
  vim.g.kwch_buf = buf
  vim.g.kwch_win = vim.fn.bufwinid(buf)
end
...
M.rand_hlgroup = function()
  ...
  return "KwChHead" .. hlgroups[math.random(1, #hlgroups)]
end
