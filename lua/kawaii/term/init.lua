...
local function save_term_info(index, val)
  local terms_list = g.kawaii_terms
  terms_list[tostring(index)] = val
  g.kawaii_terms = terms_list
end

local function opts_to_id(id)
  for _, opts in pairs(g.kawaii_terms) do
    if opts.id == id then
      return opts
    end
  end
...
M.display = function(opts)
  ...
  vim.bo[opts.buf].ft = "KwTerm_" .. opts.pos:gsub(" ", "")
  ...
  if (opts.pos == "sp" and not vim.g.kwhterm) or (opts.pos == "vsp" and not vim.g.kwvterm) or (opts.pos ~= "float") then
...
local function create(opts)
  ...
  vim.g.kwhterm = opts.pos == "sp"
  vim.g.kwvterm = opts.pos == "vsp"
end
