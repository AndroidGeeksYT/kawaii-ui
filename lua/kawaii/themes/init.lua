...
local ui = require "kawaii.themes.ui"
local state = require "kawaii.themes.state"
local colors = dofile(vim.g.base46_cache .. "colors")

state.ns = api.nvim_create_namespace "KwThemes"

if not state.val then
  state.val = require("kawaii.utils").list_themes()
  ...
  volt.mappings {
    ...
    after_close = function()
      if not state.confirmed then
        ...
        require("kawaii.themes.utils").reload_theme(theme)
      end
      require("plenary.reload").reload_module "kawaii.themes"
      vim.cmd.stopinsert()
    end,
  }

  require "kawaii.themes.mappings"
