...
local state = require "kawaii.themes.state"
local redraw = require("volt").redraw
local utils = require "kawaii.themes.utils"
local kwapi = require "kawaii.themes.api"
...
map("i", { "<C-n>", "<Down>" }, kwapi.move_down, { buffer = state.input_buf })
map("n", { "j", "<Down>" }, kwapi.move_down, { buffer = state.input_buf })
map("i", { "<C-p>", "<Up>" }, kwapi.move_up, { buffer = state.input_buf })
map("n", { "k", "<Up>" }, kwapi.move_up, { buffer = state.input_buf })
...
map({ "i", "n" }, { "<cr>" }, function()
  ...
  old_theme = '"' .. old_theme .. '"'
  require("kawaii.utils").replace_word(old_theme, '"' .. name .. '"')
